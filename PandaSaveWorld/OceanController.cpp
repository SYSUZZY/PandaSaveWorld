...
OceanController::OceanController(PhysicsEngine* physicsEngine) {
	_physicsEngine = physicsEngine;
}

void OceanController::initOcean() {
	installShaders();
	initWave();
	initGL();
}

void OceanController::renderOcean(Camera *currentCamera, float deltaTime) {
	displayFunc();
	values.time += 0.05;
}
...
static void installShaders(void)
{
	names.vertex_shader = initShader(GL_VERTEX_SHADER, "../res/shader/wave-gerstner-light.vs");
	names.fragment_shader = initShader(GL_FRAGMENT_SHADER, "../res/shader/wave-gerstner-light.fs");
...
static void initGL(void)
{
	...
	names.diffuse_texture = initTexture("../res/texture/ocean/water-texture-2.tga");
	names.uniforms.diffuse_texture = glGetUniformLocation(names.program, "textures[0]");
	glUniform1i(names.uniforms.diffuse_texture, 0);

	names.normal_texture = initTexture("../res/texture/ocean/water-texture-2-normal.tga");
