...
Flame::Flame() {
	...
	mUpdateShader = new Shader("../res/shader/Update.vs", "../res/shader/Update.fs", 
							"../res/shader/Update.gs", varyings, 7);

	//设置TransformFeedback缓存能够记录的顶点的数据类型
	mRenderShader = new Shader("../res/shader/Render.vs", "../res/shader/Render.fs");

	//设置随机纹理
	InitRandomTexture(580);
	mSparkTexture.loadTexture("../res/texture/particle.bmp");
	mStartTexture.loadTexture("../res/texture/flame.bmp");
	mRenderShader->use();
