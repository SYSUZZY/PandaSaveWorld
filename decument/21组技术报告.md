...
memset(particles, 0, sizeof(particles));//分配内存
particles[0].type = PARTICLE_TYPE_LAUNCHER;//设置第一个粒子的类型为发射器
particles[0].position = pos;//初始化一级粒子的中心粒子的位置
particles[0].lifetimeMills = 0.0f;//初始化一级粒子的中心粒子的年龄
particles[0].velocity = glm::vec3(0.0f, 0.1f, 0.0f);//初始化一级粒子的中心粒子的速度
```

4. 通过高斯分布初始化其它粒子的位置，并以粒子系统宏随机生成其它属性
...
for (int x = 0; x < nums; x++) {
		...
		record.z += center.z;
		record.y = center.y;
		particles[x].type = PARTICLE_TYPE_LAUNCHER;// 一级粒子
		particles[x].position = record;// 初始化其它发射器粒子的位置
		particles[x].velocity = DEL_VELOC*(float(rand()) / float(RAND_MAX)) + MIN_VELOC;//在最大最小速度之间随机选择
		particles[x].alpha = 1.0f;// 全部都是原始透明度，因为地盘火焰最旺
		particles[x].size = INIT_SIZE;// 发射器粒子大小
		particles[x].lifetimeMills = (MAX_LIFE - MIN_LIFE)*(float(rand()) / float(RAND_MAX)) + MIN_LIFE;//在最短最长寿命之间随机选择，作为粒子的初始化年龄
		particles[x].life = particles[x].lifetimeMills;//使粒子的年龄与寿命相同，这样发射器发射完后会立即死亡(消失)
	}
}
```

5. 通过几何着色器(geometry shader)实现一、二级粒子的不同属性实现
...
// 要输出的其余粒子的各项属性
// 包括以粒子中心形成的火焰底盘其余发射器粒子和发射器产生的二级粒子
out float Type1;
out vec3 Position1;
out vec3 Velocity1;
out float Age1;
